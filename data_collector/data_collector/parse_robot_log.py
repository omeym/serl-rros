...
def parse_robot_data(file_path):
    ...
    joint_columns_sorted = sorted(
        joint_columns, key=lambda x: int(joint_pattern.match(x).group(1))
    )
    joint_rename_map = {
        col: f"A{idx+1}" for idx, col in enumerate(joint_columns_sorted)
    }
    ...
    if "ZeitInSec" not in df.columns or "ZeitInNanoSec" not in df.columns:
        raise ValueError(
            "Required time columns 'ZeitInSec' and/or 'ZeitInNanoSec' are missing."
        )
    ...
    force_columns = {"cartForce1_X": "Fx", "cartForce1_Y": "Fy", "cartForce1_Z": "Fz"}
    torque_columns = {
        "cartTorque1_TauX": "Tx",
        "cartTorque1_TauY": "Ty",
        "cartTorque1_TauZ": "Tz",
    ...
    missing_force_columns = [
        col for col in force_columns.keys() if col not in df.columns
    ]
    missing_torque_columns = [
        col for col in torque_columns.keys() if col not in df.columns
    ]
    missing_columns = missing_force_columns + missing_torque_columns
    if missing_columns:
        raise ValueError(
            f"The following required columns are missing in the data: {missing_columns}"
        )
    ...
    selected_columns = (
        ["time"]
        + list(joint_rename_map.values())
        + list(force_columns.values())
        + list(torque_columns.values())
    )
...
def main():
    ...
    log_folder_path = os.path.join(MAIN_DIR, "11_dec_logs")
    save_df_path = os.path.join(MAIN_DIR, "processed_data")
    ...
    for file_path in log_files:
        ...
        save_file_path = os.path.join(
            save_df_path, os.path.splitext(os.path.basename(file_path))[0] + ".csv"
        )
